// ! M_Teeth_air/Default/FLocalVertexFactory/TMobileBasePassPSFMobileDirectionalLightAndSHIndirectPolicyHDRLinear64EnableLocalLights/0/MobileBasePassPixelShader.usf:Main
...
void main()
{
    ...
    highp vec3 _584 = vec4(_180, sqrt(clamp(1.0 - dot(_180, _180), 0.0, 1.0)), 1.0).xyz * Material_PreshaderBuffer_1.xyz;
    float _88 = _584.z + 1.0;
    ...
    vec3 _92 = vec4(_187, sqrt(clamp(1.0 - dot(_187, _187), 0.0, 1.0)), 1.0).xyz * vec3(_598, _598, 1.0);
    vec3 _97 = vec3(_584.xy, _88);
    ...
    highp vec3 _611 = mix(vec3(0.0, 0.0, 1.0), vec4(_194, sqrt(clamp(1.0 - dot(_194, _194), 0.0, 1.0)), 1.0).xyz, vec3(Material_PreshaderBuffer_2.y));
    vec3 _117 = vec3(_107.xy, _109);
    vec3 _121 = vec3(_611.xy * vec2(-1.0), _611.z);
    ...
    vec2 _201 = (_615.xy * vec2(2.0)) - vec2(1.0);
    highp float _618 = sqrt(clamp(1.0 - dot(_201, _201), 0.0, 1.0));
    vec3 _134 = vec3(_127.xy, _129);
    vec3 _137 = vec3(vec4(_201, _618, 1.0).xy * vec2(-1.0), _618);
    ...
    highp vec3 _623 = (-_573) + ((_144 * dot(_144, _573)) * 2.0);
    ...
    float _156 = _651.x;
    vec3 _158 = mix(mix(_145, _634.xyz * (_145 / _638.xyz), vec3(0.669921875)), (_645.xyz * (_145 * vec3(1.0380132198333740234375, 1.27521789073944091796875, 1.815602779388427734375))) * vec3(Material_PreshaderBuffer_4.z), vec3(_156));
    ...
    vec3 _216 = clamp(mix(_158, _158 * vec3((_210 <= 0.0) ? 0.0 : pow(_210, 0.300048828125), (_211 <= 0.0) ? 0.0 : pow(_211, 0.300048828125), (_208 <= 0.0) ? 0.0 : pow(_208, 0.300048828125)), vec3(_156 * clamp(1.0 - ((1.0 - _651.y) * Material_PreshaderBuffer_6.y), 0.0, 1.0))) * _167, vec3(0.0), vec3(1.0));
    float _217 = clamp((_167 * vec3(smoothstep(0.0, Material_PreshaderBuffer_6.z, dot(((vec3(dot(_573, _169)) * _169) * vec3(2.0)) - _573, _573)) * 0.449951171875)).x, 0.0, 1.0);
    highp float _680 = max(0.015625, clamp(mix(Material_PreshaderBuffer_7.x, Material_PreshaderBuffer_6.w, _156), 0.0, 1.0));
    vec3 _220 = dFdx(_144);
    vec3 _221 = dFdy(_144);
    highp float _686 = clamp(pow(max(dot(_220, _220), dot(_221, _221)), View_NormalCurvatureToRoughnessScaleBias.z), 0.001000000047497451305389404296875, 1.0);
    vec3 _225 = vec3(0.07999999821186065673828125 * _217);
    float _232 = _144.y;
    vec4 _398 = _540;
    _398.y = (-0.48860299587249755859375) * _232;
    float _233 = _144.z;
    vec4 _399 = _398;
    _399.z = 0.48860299587249755859375 * _233;
    float _234 = _144.x;
    vec4 _400 = _399;
    _400.w = (-0.48860299587249755859375) * _234;
    vec3 _235 = _144 * _144;
    vec4 _401 = _537;
    _401.x = (1.09254801273345947265625 * _234) * _232;
    vec4 _402 = _401;
    _402.y = ((-1.09254801273345947265625) * _232) * _233;
    vec4 _403 = _402;
    _403.z = 0.3153919875621795654296875 * ((3.0 * _235.z) - 1.0);
    vec4 _404 = _403;
    _404.w = ((-1.09254801273345947265625) * _234) * _233;
    vec4 _405 = _400;
    _405.x = 0.885601043701171875;
    vec3 _228 = _405.yzw * 2.09375;
    vec4 _229 = vec4(_405.x, _228.x, _228.y, _228.z);
    vec4 _230 = _404 * 0.78515625;
    float _231 = (_235.x - _235.y) * 0.4289104640483856201171875;
    vec3 _406 = vec3(0.0);
    _406.x = (dot(IndirectLightingCache_IndirectLightingSHCoefficients0(0), _229) + dot(IndirectLightingCache_IndirectLightingSHCoefficients1(0), _230)) + (IndirectLightingCache_IndirectLightingSHCoefficients2.x * _231);
    vec3 _407 = _406;
    _407.y = (dot(IndirectLightingCache_IndirectLightingSHCoefficients0(1), _229) + dot(IndirectLightingCache_IndirectLightingSHCoefficients1(1), _230)) + (IndirectLightingCache_IndirectLightingSHCoefficients2.y * _231);
    vec3 _408 = _407;
    _408.z = (dot(IndirectLightingCache_IndirectLightingSHCoefficients0(2), _229) + dot(IndirectLightingCache_IndirectLightingSHCoefficients1(2), _230)) + (IndirectLightingCache_IndirectLightingSHCoefficients2.z * _231);
    vec3 _226 = max(vec3(0.0), _408);
    vec4 _396;
    do
    {
        ...
        {
            _396 = vec4(IndirectLightingCache_DirectionalLightShadowing, 1.0, 1.0, 1.0);
            break;
        }
        _396 = vec4(1.0);
        break;
    ...
    float _265 = mix(1.0, dot(_396, _746), dot(_746, vec4(1.0)));
    highp float _750 = max(0.0, dot(_144, normalize(_573 + MobileDirectionalLight_DirectionalLightDirectionAndShadowTransition.xyz)));
    uint _753 = uint(((vec4(_539.x, _539.y, _539.z, vec4(1.0, 1.0, 1.0, Material_PreshaderBuffer_7.y).w) * View_bSubsurfacePostprocessEnabled).w * 255.0) + 0.5);
    ...
    float _283 = clamp(_680 * (_764.y * 2.0), 0.0, 1.0);
    float _269 = mix(_767, _283, _279);
    float _270 = dot(_144, MobileDirectionalLight_DirectionalLightDirectionAndShadowTransition.xyz);
    highp float _774 = float(_753);
    highp vec2 _783 = (vec2(uvec2(0u, _753)) + vec2(0.5)) * View_SSProfilesTextureSizeAndInvSize.zw;
    highp vec3 _786 = vec3(1.0) * max(0.0, dot(_144, MobileDirectionalLight_DirectionalLightDirectionAndShadowTransition.xyz));
    vec3 _286 = _216 * 0.318407952785491943359375;
    highp float _791 = 1.0 - (_750 * _750);
    float _287 = _767 * _767;
    highp float _792 = _750 * _287;
    highp float _794 = _287 / (_791 + (_792 * _792));
    float _291 = _283 * _283;
    highp float _795 = _750 * _291;
    highp float _797 = _291 / (_791 + (_795 * _795));
    ...
    float _306 = _305.x;
    float _309 = exp2((-9.28125) * clamp(abs(dot(_144, _573)) + 1.0013580322265625e-05, 0.0, 1.0));
    vec2 _316 = (vec2(-1.0400390625, 1.0400390625) * ((min(_306 * _306, _309) * _306) + _305.y)) + _305.zw;
    float _300 = clamp(_217 * 4.0, 0.0, 1.0);
    ...
    {
        _397 = textureLod(SPIRV_Cross_CombinedMobileReflectionCapture_TextureMobileReflectionCapture_TextureSampler, _623, (MobileReflectionCapture_Params.y - 1.0) - (1.0 - (1.2001953125 * log2(max(_680, 0.00100040435791015625))))).xyz * View_SkyLightColor.xyz;
    }
    else
    {
        _397 = textureLod(SPIRV_Cross_CombinedMobileReflectionCapture_TextureMobileReflectionCapture_TextureSampler, _623, (View_ReflectionCubemapMaxMip - 1.0) - (1.0 - (1.2001953125 * log2(max(_680, 0.00100040435791015625))))).xyz * MobileReflectionCapture_Params.w;
    }
    vec4 _340 = (vec4(-1.0, -0.027496337890625, -0.57177734375, 0.022003173828125) * _680) + vec4(1.0, 0.042510986328125, 1.0400390625, -0.040008544921875);
    float _341 = _340.x;
    ...
    highp vec3 _912;
    _912 = ((_226 * _216) + ((((mix(_786 * _265, textureLod(SPIRV_Cross_CombinedView_SSProfilesPreIntegratedTextureView_SSProfilesPreIntegratedSampler, vec3((max(_270 - (max(2.0 * _270, 0.4000000059604644775390625) * (1.0 - sqrt(_265))), -1.0) * 0.5) + 0.5, _686, _774), 0.0).xyz, textureLod(SPIRV_Cross_CombinedView_SSProfilesTextureView_SSProfilesSampler, _783, 0.0).xyz) * _286) * 1.0) + ((_786 * (((_225 * _316.x) + vec3(_300 * _316.y)) * (mix(min(_794 * _794, 2048.0), min(_797 * _797, 2048.0), _279) * ((_269 * 0.25) + 0.25)))) * MobileDirectionalLight_DirectionalLightDistanceFadeMADAndSpecularScale.z)) * (MobileDirectionalLight_DirectionalLightColor.xyz * _265))) + (((_397 * mix(1.0, min(dot(_226, vec3(0.300048828125, 0.58984375, 0.1099853515625)) / max(MobileReflectionCapture_Params.x, 9.9999997473787516355514526367188e-05), View_ReflectionEnvironmentRoughnessMixingScaleBiasAndLargestWeight.z), smoothstep(0.0, 1.0, clamp((_680 * View_ReflectionEnvironmentRoughnessMixingScaleBiasAndLargestWeight.x) + View_ReflectionEnvironmentRoughnessMixingScaleBiasAndLargestWeight.y, 0.0, 1.0)))) * 1.0) * ((_225 * _349.x) + vec3(_300 * _349.y)));
    highp vec3 _913;
    SPIRV_CROSS_LOOP
    for (uint _915 = 0u; _915 < _910; _912 = _913, _915++)
    {
        ...
        if (_999 > 0.0)
        {
            float _353 = mix(1.0, dot(_396, _967), dot(_967, vec4(1.0)));
            highp float _1007 = max(0.0, dot(_144, normalize(_573 + _971)));
            ...
            float _367 = clamp(_680 * (_1009.y * 2.0), 0.0, 1.0);
            float _354 = mix(_1012, _367, _363);
            float _355 = dot(_144, _971);
            highp vec3 _1024 = vec3(1.0) * max(0.0, dot(_144, _971));
            highp float _1029 = 1.0 - (_1007 * _1007);
            float _370 = _1012 * _1012;
            highp float _1030 = _1007 * _370;
            highp float _1032 = _370 / (_1029 + (_1030 * _1030));
            float _374 = _367 * _367;
            highp float _1033 = _1007 * _374;
            highp float _1035 = _374 / (_1029 + (_1033 * _1033));
            ...
            _1046 = (((mix(_1024 * _353, textureLod(SPIRV_Cross_CombinedView_SSProfilesPreIntegratedTextureView_SSProfilesPreIntegratedSampler, vec3((max(_355 - (max(2.0 * _355, 0.4000000059604644775390625) * (1.0 - sqrt(_353))), -1.0) * 0.5) + 0.5, _686, _774), 0.0).xyz, textureLod(SPIRV_Cross_CombinedView_SSProfilesTextureView_SSProfilesSampler, _783, 0.0).xyz) * _286) * 1.0) + ((_1024 * (((_225 * _395.x) + vec3(_300 * _395.y)) * (mix(min(_1032 * _1032, 2048.0), min(_1035 * _1035, 2048.0), _363) * ((_354 * 0.25) + 0.25)))) * 1.0)) * ((_928.xyz * _999) * _353);
        }
        else
        {
            _1046 = vec3(0.0);
    ...
    vec3 _72 = _912 * 1.0;
    vec4 _409 = vec4(_72.x, _72.y, _72.z, _537.w);
    _409.w = 0.0;
    highp vec3 _1050 = min((_409.xyz * View_PreExposure).xyz, vec3(32512.0, 32512.0, 32256.0));
    out_var_SV_Target0 = vec4(_1050.x, _1050.y, _1050.z, _409.w);
